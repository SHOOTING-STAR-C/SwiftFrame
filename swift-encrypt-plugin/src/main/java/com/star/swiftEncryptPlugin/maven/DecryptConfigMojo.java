 ...
 */
@Mojo(name = "decrypt-config")
@Setter
public class DecryptConfigMojo extends AbstractMojo {
    private static final Pattern DNC_PATTERN = Pattern.compile("ENC\\(([^)]+)\\)");
    ...
    public void execute() throws MojoExecutionException {
        ...
        try {
            ...
            try (FileWriter writer = new FileWriter(configFile)) {
                ...
                getLog().info("Successfully decrypted configuration to: " + configFile.getPath());
            }
        } catch (Exception e) {
     ...
     */
    private void decryptConfig(Map<String, Object> config, String aesKey) throws Exception {
        for (Map.Entry<String, Object> entry : config.entrySet()) {
            if (entry.getValue() instanceof String) {
                entry.setValue(decryptConfig((String) entry.getValue(), aesKey));
            } else if (entry.getValue() instanceof Map) {
                @SuppressWarnings("unchecked")
                Map<String, Object> nestedMap = (Map<String, Object>) entry.getValue();
                decryptConfig(nestedMap, aesKey);
            }
        }
    }
     ...
     * @throws Exception Exception
     */
    private String decryptConfig(String value, String aesKey) throws Exception {
        Matcher matcher = DNC_PATTERN.matcher(value);
        if (matcher.find()) {
            String plainText = matcher.group(1);
            String encrypted = "DEC(" + AesGmcUtil.decrypt(plainText, aesKey) + ")";
     ...
     * 解析配置文件路径
     */
    public void resolveConfigFile() throws MojoExecutionException {
        // 2. 处理绝对路径
